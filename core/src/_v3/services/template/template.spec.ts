...
test('steps() returns the inline-worker steps + the build loop edge', () => {
  ...
  expect(plan.steps[0]).toMatchObject({ worker: 'build-implement', type: 'agent' })
...
task:
  build:
    steps:
      - { name: lint, run: "bun run lint", after: implement }
`
