...
import { z } from 'zod'
import { phaseStatusValues, phaseExecuteValues } from '../shared/statuses.js'
...
export const PhaseExecute = z.enum(phaseExecuteValues)
...
export const PhaseNodeSchema = z.strictObject({
  ...
  // execute (how the phase builds): sequential | workflow — plan/refine sets it (intra-phase
  // AC fan-out). Optional, no injected default (round-trips identical).
  execute: PhaseExecute.optional(),
  // depends_on: other phase slugs that must finish first — plan/refine sets it; ready-phases
  // honours it so independent phases can build in parallel (multi-phase fan-out).
