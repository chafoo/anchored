// _v3/services/store/scope/safe-write.ts — the atomic-write dance, store-internal. The ONE
...
// under the lock means a concurrent writer landed (the parallel epic fan-out) → reject loudly
// instead of clobbering. fs/lock/rand/pid are injected (no node:fs here beyond pure path).
