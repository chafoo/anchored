...
- **Build phase by phase, hands-off** — `task build` is the recursion edge `build.each: phase`: it walks each phase from start to green, running the implementer plus the two always-on gates (task-validate, code-validate), with a failures-driven re-do loop bounded by `retry_limit`.
