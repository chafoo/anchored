...
flowchart TD
  ...
  g2["code-validate — check the code against the phase rules"]
...
- **Carry the rules that apply to it** — collection `rule` holds the per-phase rules distributed during planning, which the code-validate gate checks the implementation against and can use to bounce a criterion that was otherwise evidenced.
