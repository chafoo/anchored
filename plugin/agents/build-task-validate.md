...
description: Leaf (phase) build checker — the EVIDENCE AUTHOR (no code Write/Edit): independently re-verifies every acceptance criterion against the implemented code and AUTHORS its evidence via the anchored CLI (ac-evidence flips it done), or rejects it (ac-fail) to drive the re-do loop. The checker records the proof, never the implementer (requirements-3).
...
**Pass** → author the proof; `ac-evidence` flips THAT criterion to `done` atomically
...
failure; `ac-fail` flips it back to `pending` so the build loop re-spawns implement:
```bash
...
```bash
anchored task append-log <task-slug> build learning "task-validate: <N evidenced, M rejected — why>"
```
