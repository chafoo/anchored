...
`child-add` takes the **goal** (3rd arg) and the **depends_on dependency edge** (4th arg,
...
`child-set-field` — the generic set-field can't address an array element:

```bash
anchored epic child-set-field <slug> <stub-slug> depends_on '["<dep-slug>"]'
anchored epic child-set-field <slug> <stub-slug> goal "<new goal>"
```
