...
| `validate` | `anchored validate <slug> [--gate <g>] [--snapshot <ref>]` | Returns the **validation packet** for ONE validator spawn: the gate's open/failed criteria, the resolved setup instructions, the snapshot (minted `snap-…` token, or your `--snapshot` verbatim), rigor, fields. The CLI never spawns — the skill does. Gates are setup-homogeneous. |
| `evidence` | `anchored evidence <slug> <criterion> --snapshot <s> [--grounded <proof>] [--verdict <v>]` | VALIDATOR-ONLY. Flips to `done`. Needs grounded output or a verdict — the schema refuses otherwise. |
